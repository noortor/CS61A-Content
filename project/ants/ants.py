...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if hasattr(self, "turn_around") and self.turn_around:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p_check = self.place
        count = 0
        while p_check is not beehive:
            if p_check.bees and count <= self.max_range and count >= self.min_range:
                return random_or_none(p_check.bees)
            p_check = p_check.entrance
            count += 1
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt protects the colony by blocking off Bees."""
    name = 'Wall'
    food_cost = 4
    implemented = True
    def __init__(self):
...
class ScubaThrower(ThrowerAnt):
    """A Thrower Ant that won't die in water"""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true:
            ...
            while place_check:
                if place_check.ant:
                    if not hasattr(place_check.ant, 'doubled'):
                        place_check.ant.damage *= 2
                        place_check.ant.doubled = True
                    if place_check.ant.is_container and place_check.ant.contained_ant: 
                        if not hasattr(place_check.ant.contained_ant, 'doubled'):
                            place_check.ant.contained_ant.damage *= 2
                            place_check.ant.contained_ant.doubled = True
...
def apply_effect(effect, bee, duration):
    ...
    def create_action(colony):
        nonlocal duration
        if not duration:
            if hasattr(bee, "turn_around") and bee.turn_around:
                bee.turn_around = False
